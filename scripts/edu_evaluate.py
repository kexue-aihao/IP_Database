# -*- coding: utf-8 -*-
"""Evaluate v1 vs v2 on edu_ipv4 anchors only (matches original baseline)."""
...
OUT = r'E:/IP_Database/output/edu_precision.json'
def to_float(v):
    try:
...
def norm(name):
    ...
    return name
# Load edu_ipv4 anchors with complete data
anchors = []
with open(ANCHOR_PATH, encoding='utf-8') as f:
    reader = csv.DictReader(f)
    for r in reader:
        if r.get('type','') != 'edu_ipv4': continue
        ...
        if prov and city and lat is not None and lng is not None:
            anchors.append({'ip': ip, 'province': prov, 'city': city, 'lat': lat, 'lng': lng})
print(f'edu_ipv4 anchors with complete data: {len(anchors)}')
...
report = {'anchors': len(anchors), 'per_mmdb': {}}
